...
const Offers = () => {
  const offers = [
    ...
    {
      title: "Agoda",
      description: "Best Prices Guaranteed with Deals, Special Member Prices.",
      logo: agodaLogo,
      rating: 3,
      eligibility: "متطلبات التأهيل",
      minimumAmount: 8000,
  ...
  const [signedOffers, setSignedOffers] = useState([]);
  ...
  const handleSignatureClick = (index) => {
    ...
    setTimeout(() => {
      ...
      setSignedOffers((prev) => [...prev, index]);
    }, 2000);
  };
    ...
    <div className="mx-auto py-8 px-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {offers.map((offer, index) => (
          <div 
            key={index} 
            className="bg-white rounded-lg shadow-md overflow-hidden border border-gray-100 hover:shadow-lg transition-shadow duration-300 p-6 flex flex-col justify-between h-[400px]"
          >
            {/* Top content wrapper */}
            <div>
            ...
            <Link href='/tasks' className="mt-auto w-full">
              <button 
                ...
                disabled={(signID !== -1 && signID !==index) || signedOffers.includes(index)}
